...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        candidate_items = (
            Item.objects.filter(usage_history__discrimination_index__isnull=False)
            .distinct()
        )
        ...
        for item in candidate_items:
            recent_usages = list(
                item.usage_history.filter(discrimination_index__isnull=False)
                .order_by("-recorded_at")[:2]
            )
            if len(recent_usages) < 2:
                continue
            if all(
                float(usage.discrimination_index) < threshold
                ...
                if not item.quality_flagged:
                    ...
                    flagged_count += 1
                    self.stdout.write(
                        f"Flagged item {item.id} for moderator review."
                    )
