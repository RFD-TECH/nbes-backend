"""shared/integrations.py — System 17 client.
...
class System17Response:
    ...
    @classmethod
    def from_http(cls, response: requests.Response, correlation_id: str) -> "System17Response":
        try:
            body = response.json()
        except ValueError:
            body = {}
...
def call_system_17(
    ...
    *,
    ...
    request_timeout = timeout if timeout is not None else getattr(
        settings, "SYSTEM_17_TIMEOUT_SECONDS", 5
    )

    last_response: requests.Response | None = None
    for attempt in range(1, max_retries + 1):
        headers = _build_headers(body, idempotency_key, correlation_id)
        try:
        ...
        logger.info(
            ...
            attempt, last_response.status_code, endpoint,
        )
        time.sleep(_backoff_seconds(attempt))
...
def _build_headers(body: str, idempotency_key: str, correlation_id: str) -> dict:
